...
, benchmark, syscall, systemd, xfce ? null, xorg ? null, pkgs }:
...
EOF
      ...
      # XFCE4 Session
      xfce4_session_mappings="bin:$out/usr/bin etc:$out/etc lib:$out/usr/lib share:$out/usr/share"
      process_package_mappings "${pkgs.xfce.xfce4-session}" "$xfce4_session_mappings" "XFCE4-Session"
      ...
      glib_mappings="bin:$out/usr/bin"
      process_package_mappings "${pkgs.glib}" "$glib_mappings" "GLib"
      ...
      gvfs_mappings="lib:$out/usr/lib libexec:$out/usr/libexec share:$out/usr/share"
      process_package_mappings "${pkgs.gvfs}" "$gvfs_mappings" "GVFS"
      ...
      # xfce.exo
      exo_mappings="bin:$out/usr/bin lib:$out/usr/lib share:$out/usr/share"
      process_package_mappings "${pkgs.xfce.exo}" "$exo_mappings" "XFCE-Exo"
